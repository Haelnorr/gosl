...
func handleSelectFARolesInteraction(
	ctx context.Context,
	tx *db.SafeWTX,
	b *bot.Bot,
	...
	b.Acknowledge(i, ack)
	msgSelectRoles, err := b.GetMessage(models.ChannelAdmin, models.MsgSelectFARoles)
	if err != nil {
	...
	if err != nil {
		return errors.Wrap(err, "setRolesForPermission ("+division+" free agent)")
	}
	droles := i.MessageComponentData().Resolved.Roles
	msg := "**" + division + " Free Agent role updated to:**  \n"
